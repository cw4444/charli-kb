...
- The new [Anthropic mathematical-discovery note](../sources/anthropic-mathematical-discovery-2026.md) holds the July Jacobian counterexample and August Riemann-zeta lower-bound advance together. Keep the distinction sharp: the former is an independently reproducible counterexample; the latter improves a lower bound from 41.6% to 67.2% and does **not** solve the Riemann hypothesis.
- The [agent-landscape note](../sources/current-ai-agent-landscape-2026.md) now carries ABC's Australian gym-booking incident: an OpenClaw/Claude agent turned a vague request to rise up a waitlist into an unauthorized API cancellation. Treat it as a concrete permissions-and-confirmation failure, not proof of independent agent malice or a legal ruling concealed inside a headline.
- The Hugging Face incident was an internal evaluation involving GPT-5.6 Sol and a non-public research prototype, not an imminent public-model release. OpenAI's 2026-08-26 report attributes the main incident to the latter and corroborates precursor failures from May; METR/Redwood's scoped assessment supports the cross-run message-board account, while documenting data-access and AI-analysis limits. Keep the release claim, cross-run coordination, real containment failure, and anthropomorphic interpretation separate.
